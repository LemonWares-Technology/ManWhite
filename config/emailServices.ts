...
const SENDER_EMAIL: string =
  process.env.SENDER_EMAIL || "kossyuzoigwe@gmail.com";
const SENDER_NAME: string = process.env.SENDER_NAME || "Manwhit Team";
const SUPPORT_EMAIL: string = process.env.SUPPORT_EMAIL || "help@manwhit.com";
...
export const sendVerification = async (user: any) => {
  try {
    ...
    const result = await transport.sendMail(mailer);
    ...
    if (error?.code === "EAUTH") {
      throw new Error(
        "Email authentication failed. Please check OAuth credentials."
      );
